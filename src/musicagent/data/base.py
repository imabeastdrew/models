...
class BaseDataset(Dataset, ABC):
    ...
    def __init__(self, config: DataConfig, split: str = "train"):
        ...
        # Load the single vocabulary generated by preprocessing.
        if not config.vocab.exists():
            raise FileNotFoundError(
                f"Vocabulary not found at {config.vocab}. "
                "Please preprocess the dataset with scripts/preprocess.py."
            )

        with open(config.vocab) as f:
        ...
        # Compute vocab size dynamically from max ID + 1 to handle sparse layouts.
        if self.token_to_id:
            self.vocab_size: int = max(self.token_to_id.values()) + 1
        else:
            self.vocab_size = 0

        # Melody and chord "views" are derived from token naming conventions.
        self._melody_vocab_ids: set[int] = {
            idx for tok, idx in self.token_to_id.items() if tok.startswith("pitch_")
        }
        self._chord_vocab_ids: set[int] = {
            idx
            for tok, idx in self.token_to_id.items()
            if (tok.endswith("_on") or tok.endswith("_hold")) and ":" in tok
        }
    ...
    def _build_transposition_tables(self) -> None:
        ...
        # native vocab space.
        vocab_len = max(self.vocab_size, 1)
        self.melody_transpose_table = np.arange(vocab_len, dtype=np.int32)
        self.melody_transpose_table = np.tile(self.melody_transpose_table, (num_offsets, 1))

        self.chord_transpose_table = np.arange(vocab_len, dtype=np.int32)
        ...
        for semitones in range(-max_transpose, max_transpose + 1):
            ...
            for token, token_id in self.token_to_id.items():
                ...
                try:
                    ...
                    new_id = self.token_to_id.get(new_token)
